...
class ProductDetailScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fetchProductFuture = useMemoized(() async {
      final product = await context.read<ProductListCubit>().fetchProduct(
        id: id,
      );
      if (context.mounted) {
        context.read<RecentlyViewedProductListCubit>().addProduct(product);
      }
      return product;
    }, [id]);
                        ...
                        children: product.sizes
                            .map(
                              (size) => RawMaterialButton(
                                ...
                                shape: CircleBorder(
                                  side: BorderSide(
                                    width: size == selectedSize.value ? 3 : 1,
